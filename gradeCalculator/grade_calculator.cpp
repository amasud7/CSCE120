...
int main() {
    ...
    while (!line.empty()) {
        ...
        if (category == "exam") {
            ...
            // if no final exam in data it is zero
    ...
    exam_average = (final_exam + exam) / (final_count + exam_count);
    double ex_avg = exam / exam_count;
    if (final_exam > ex_avg) {
    ...
    // these have plus 15% added

    if (read_count != 0) {
        reading = (reading_score / read_count) + 15;
        if (reading > 100) {
            reading = 100;
        }
    }
    ...
    if (engagement_count != 0) {
        engagement = (engagement_score / engagement_count) + 15;
        if (engagement > 100) {
            engagement = 100;
        }
    }
    ...
    double weighted_total = (0.4 * hw_average) + (0.4 * exam_average) + (0.1 * lw_average) + (0.05 * reading) + (0.05 * engagement);
